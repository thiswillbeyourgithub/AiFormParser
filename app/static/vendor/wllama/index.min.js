...
var LIBLLAMA_VERSION = "b6-7229209";
...
var Wllama = class {
  constructor(pathConfig, wllamaConfig = {}) {
    ...
    // Whether the last loadModelFromUrl() resolved to the compat
    // (ASYNCIFY, CPU-only, no WebGPU) bundle rather than the main
    // (WebGPU/JSPI/memory64) bundle. Public so callers and diagnostics can
    // report the active bundle. Undefined-then-boolean: only meaningful
    // after a load has started.
    __publicField(this, "useCompat", false);
  ...
  loadModel(_0) {
    return __async(this, arguments, function* (ggufBlobsOrModel, params = {}) {
      ...
      if (needCompat() || this.config.forceCompat) {
        if (!this.compat) {
          this.logger().warn(
      ...
      this.useCompat = workerResources.compat;
      this.logger().warn(
        this.useCompat ? "==================== wllama BUNDLE = COMPAT (ASYNCIFY, CPU-only, NO WebGPU) ====================" : "==================== wllama BUNDLE = MAIN (WebGPU / JSPI / memory64) ===================="
      );
      ...
      const loadResult = yield this.proxy.wllamaAction("load", {
        ...
        // suppress llama.cpp "--cache-idle-slots requires --kv-unified" warning until kv_unified support
  ...
  createCompletionImpl(options) {
    return __async(this, null, function* () {
      ...
      return yield this.getResponse(options, isStream);
    });
  }
  /**
